...
describe('computeAutoTune', () => {
  ...
  it('derives chunk size from median length with 10% headroom', () => {
    ...
    const result = computeAutoTune({ messageCount: 100, chunkCount: 300, medianChunkLen: 700 }, 'gpt-4o');
    expect(result.chunkSize).toBe(800);
  });

  it('clamps chunk size to 400 minimum', () => {
    const result = computeAutoTune({ messageCount: 10, chunkCount: 30, medianChunkLen: 100 }, 'gpt-4o');
    expect(result.chunkSize).toBe(400);
  });

  it('clamps chunk size to 1600 maximum', () => {
    const result = computeAutoTune({ messageCount: 10, chunkCount: 30, medianChunkLen: 9000 }, 'gpt-4o');
  ...
  it('recognises gpt-4 (8k context) and caps budget at 12k minimum', () => {
    const result = computeAutoTune({ messageCount: 100, chunkCount: 300, medianChunkLen: 700 }, 'gpt-4');
    // 32768 * 0.25 = 8192 → clamped to minimum 12000
    expect(result.contextBudget).toBe(12000);
  });

  it('recognises claude context window', () => {
    const result = computeAutoTune({ messageCount: 100, chunkCount: 300, medianChunkLen: 700 }, 'claude-3-opus');
    // 800000 * 0.25 = 200000 → clamped to 128000
    expect(result.contextBudget).toBe(128000);
  });

  it('uses safe default for unknown models', () => {
    const result = computeAutoTune({ messageCount: 10, chunkCount: 30, medianChunkLen: 700 }, 'unknown-model-xyz');
  ...
  it('keeps K within [3, 20]', () => {
    const result = computeAutoTune({ messageCount: 5, chunkCount: 10, medianChunkLen: 100 }, 'gpt-4o');
...
describe('autoTuneDescription', () => {
  it('returns empty string when no messages indexed', () => {
    expect(autoTuneDescription({ messageCount: 0, chunkCount: 0, medianChunkLen: 0 }, 'gpt-4o')).toBe('');
  });

  it('includes message and segment counts', () => {
    const desc = autoTuneDescription({ messageCount: 500, chunkCount: 1500, medianChunkLen: 720 }, 'gpt-4o-mini');
